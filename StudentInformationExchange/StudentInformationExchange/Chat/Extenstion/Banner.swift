...
func showInAppNotification(messageFrom:String,documentId:String,dataType:String,title:String,body:String) {
       ...
       let announce = Announcement(
             ...
             action: { (type, string, announcement) in
                   ...
                   if type == CallbackType.tap{
                       ...
                       handelNotificationTap(documentId:documentId,dataType:dataType)
       ...
       //InAppNotify.Show(announce, to:  UIApplication.topViewController()!)
       
    showSuccessBanner(from: messageFrom, message: title)
...
class NotificationManagerAppClosed {
   
   static let shared =  NotificationManagerAppClosed()
   
   let defaults = UserDefaults.standard

   func handelNotification(launchOptions:[UIApplication.LaunchOptionsKey: Any]?) {
       
       if let options = launchOptions {
           let info = options[.remoteNotification]
           if (info != nil) {
...
//                    let body = alert?["body"] ?? ""
                   let dataType = userInfo["data_type"] as? String
                   let documentId = userInfo["class_id"] as? String
...
func handelNotificationTap(documentId:String,dataType:String) {
      
  
        
//         if let topVC = UIApplication.topViewController() {
//
//             let vc = UIStoryboard.liveClass
...
func showSuccessBanner(from:String,message: String) {
    let banner = Banner(title:from, subtitle: message + "\n", image: nil, backgroundColor: UIColor.appColor)
    banner.show(duration: 2.0)
}

func showErrorBanner(message: String) {
    let banner = Banner(title: "", subtitle: message + "\n", image: nil, backgroundColor: UIColor.init(named: "eb5757")!)
